...
def openkp_loader(mode, source_dataset_dir):
    ''' load source OpenKP dataset :'url', 'VDOM', 'text', 'KeyPhrases', 'title' '''
    ...
    with codecs.open(source_path, "r", "utf-8") as corpus_file:
        for idx, line in enumerate(tqdm(corpus_file, ncols=80)):
...
def openkp_refactor(examples, mode):
    ...
    for idx, ex in enumerate(tqdm(examples, ncols=80)):
        doc_words, word2block, block_features = prepro_utils.refactor_text_vdom(text=ex['text'],
                                                                                VDOM=ex['VDOM'])
        title_words = prepro_utils.refractor_title(title=ex['title'])
        data = {}
        data['url'] = ex['url']
...
def kp20k_refactor(src_trgs_pairs, mode, valid_check=True):
    ...
    # ---------------------------------------------------------------------------------------
    return_pairs = []
    for idx, (src, trgs) in enumerate(tqdm(src_trgs_pairs, ncols=80)):
...
def filter_openkp_absent(examples):
    ...
    for idx, ex in enumerate(tqdm(examples, ncols=80)):
        ...
        lower_words = [t.lower() for t in ex['title_words'] + ["[title]"] + ex['doc_words']]
        ...
        data['url'] = ex['url']
        data['index'] = idx
        data['title_words'] = ex['title_words']
...
def save_ground_truths(examples, filename, kp_key):
    with open(filename, 'w', encoding='utf-8') as f_pred:
        for ex in tqdm(examples, ncols=80):
            data = {}
            data['url'] = ex['url']
