...
from torch.nn import MarginRankingLoss, CrossEntropyLoss, NLLLoss
...
class RobertaForCnnGramKernelRanking(BertPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.cnn2gram = NGramers(input_size=config.hidden_size + 18,
...
class RobertaForChunkTFRanking(RobertaForCnnGramKernelRanking):
    ...
    def forward(self, visual_input, meta_input, input_ids, attention_mask, valid_ids, active_mask, valid_output, labels=None, chunk_labels=None, chunk_mask=None):
        ...
        if labels is not None and chunk_labels is not None:
            ...
            # *************************************************************************************
            # [1] Chunk Loss
            Chunk_Loss_Fct = NLLLoss(reduction='mean')

            active_chunk_loss = chunk_mask.view(-1) != -1
            chunk_logits = torch.cat(
                [self.chunk_classifier[i](cnn_outputs).unsqueeze(1) for i in range(4)],
                1
            )  # shape = (batch_size * num_gram, 4, 2)
            chunk_logits = F.softmax(chunk_logits, -1)
            chunk_logits = torch.log(torch.sum(chunk_logits * pred_mask, 1))  # (batch_size * num_gram, 2)
            ...
            flag = torch.FloatTensor([1]).to(total_scores.device)
