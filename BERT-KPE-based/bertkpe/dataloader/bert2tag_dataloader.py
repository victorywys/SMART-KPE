...
def bert2tag_preprocessor(examples, tokenizer, max_token, pretrain_model, snapshot_path, mode, max_phrase_words, stem_flag=False):
...
def bert2tag_converter(index, ex, tokenizer, mode, max_phrase_words):
    ...
    if mode == 'train':
        ...
        return index, src_tensor, valid_mask, orig_doc_len, label_tensor, visual, meta

    else:
        return index, src_tensor, valid_mask, orig_doc_len, visual, meta
...
def batchify_bert2tag_features_for_train(batch):
    ...
    visual_feat = [ex[5] for ex in batch]
    meta_feat = [ex[6] for ex in batch]
    ...
    return visual_input, meta_input, input_ids, input_mask, valid_ids, active_mask, valid_output, labels, ids
...
def batchify_bert2tag_features_for_test(batch):
    ...
    valid_mask = [ex[2] for ex in batch]
    doc_word_lens = [ex[3] for ex in batch]
    visual_feat = [ex[4] for ex in batch]
    meta_feat = [ex[5] for ex in batch]
    ...
    valid_output = torch.zeros(len(docs), max_word_len, bert_output_dim + 18)
    return visual_input, meta_input, input_ids, input_mask, valid_ids, active_mask, valid_output, doc_word_lens, ids
