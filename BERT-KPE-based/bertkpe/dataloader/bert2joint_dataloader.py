...
# -------------------------------------------------------------------------------------------
# -------------------------------------------------------------------------------------------
def bert2joint_preprocessor(examples, tokenizer, max_token, pretrain_model, snapshot_path, mode, max_phrase_words, stem_flag=False):
    logger.info('start preparing (%s) features for bert2joint (%s) ...' % (mode, pretrain_model))
    
    overlen_num = 0
    new_examples = []    
    for idx, ex in enumerate(tqdm(examples, ncols=80)):
        
        # tokenize
        titled_doc = ex['title_words'] + ["[title]"] + ex['doc_words']

        tokenize_output = loader_utils.tokenize_for_bert(doc_words=titled_doc,
                                                         tokenizer=tokenizer)
        ...
        if snapshot_path is not None:
            f_name = os.path.join(snapshot_path, f"{ex['index']}.npy")
            if os.path.isfile(f_name):
                new_ex['snapshot'] = np.mean(np.load(f_name).reshape(4, -1), 0)
            else:
                new_ex['snapshot'] = np.zeros((512,))
            new_ex['snapshot'] = new_ex['snapshot'].tolist()
        else:
            new_ex['snapshot'] = []
        ...
        # create visual input
        new_ex['visual'] = np.zeros((len(new_ex['tokens']), 18), dtype=np.float)
        for tok_id in range(len(new_ex['tokens'])):
            ori_pos = tokenize_output['tok_to_orig_index'][tok_id] - len(ex['title_words']) - 1
            if ori_pos >= 0:
                feature = ex['block_features'][ex['word2block'][ori_pos]]
                new_ex['visual'][tok_id, :] = np.array(feature[0:6] + feature[7:16] + feature[17:20])
        new_ex['visual'] = new_ex['visual'].tolist()

        new_ex['valid_mask'] = tokenize_output['valid_mask'][:max_token]
        new_ex['doc_words'] = titled_doc[:max_word]
...
def bert2joint_converter(index, ex, tokenizer, mode, max_phrase_words):
    ...
    visual = torch.cat((torch.zeros(1, 18), torch.tensor(ex['visual']), torch.zeros(1, 18)), 0)
    ...
    if mode == 'train':
        ...
        return index, src_tensor, valid_mask, mention_lists, orig_doc_len, max_phrase_words, label, chunk_label, visual, meta
    
    else:
        tot_phrase_len = len(ex["phrase_list"])
        return index, src_tensor, valid_mask, mention_lists, orig_doc_len, max_phrase_words, tot_phrase_len, visual, meta
...
def batchify_bert2joint_features_for_train(batch):
    ...
    visual_feat = [ex[8] for ex in batch]
    meta_feat = [ex[9] for ex in batch]
...
#     active_mask = torch.ByteTensor(len(docs), max_diff_gram_num, max_ngram_length).fill_(1) # Pytorch Version = 1.1.0
    ...
    valid_output = torch.zeros(len(docs), max_word_len, bert_output_dim + 18)
    ...
    visual_max_length = max([vis.size(0) for vis in visual_feat])
    visual_input = torch.FloatTensor(len(visual_feat), visual_max_length, 18).zero_()
    for i, v in enumerate(visual_feat):
        visual_input[i, :v.size(0), :].copy_(v)

    # ---------------------------------------------------------------
    # meta feature tensor
    meta_input = torch.stack(meta_feat)

    return visual_input, meta_input, input_ids, input_mask, valid_ids, active_mask, valid_output, ngram_label, chunk_label, chunk_mask, ids
...
def batchify_bert2joint_features_for_test(batch):
    ...
    visual_feat = [ex[7] for ex in batch]
    meta_feat = [ex[8] for ex in batch]
...
#     active_mask = torch.ByteTensor(len(docs), max_diff_gram_num, max_ngram_length).fill_(1) # Pytorch Version = 1.1.0
    ...
    # meta feature tensor
    meta_input = torch.stack(meta_feat)
    return visual_input, meta_input, input_ids, input_mask, valid_ids, active_mask, valid_output, phrase_list_lens, ids
